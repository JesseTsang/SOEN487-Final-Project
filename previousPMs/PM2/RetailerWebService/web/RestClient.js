...
// Model for Retailer entity
    models.Retailer = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/RetailerWebService/webresources/retailer.entity.retailer/",
        idAttribute: 'id',
        defaults: {
            quantity: "",
            retailerid: ""
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/RetailerWebService/webresources/retailer.entity.retailer/';
            }
    ...
    // Collection class for Retailer entities
    models.RetailerCollection = Backbone.Collection.extend({
        model: models.Retailer,
        url: "http://localhost:8080/RetailerWebService/webresources/retailer.entity.retailer/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.RetailerCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-retailer-list-itemis template identifier for item
                        templateName: '#tpl-retailer-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.retailer = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.retailer,
                    // tpl-retailer-details is template identifier for chosen model element
                    templateName: '#tpl-retailer-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Retailer();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-retailer-details is a template identifier for chosen model element
                templateName: '#tpl-retailer-details',
